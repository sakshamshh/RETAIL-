...
def get_hourly_traffic(camera=None):
    ...
    if camera:
        c.execute("""
            SELECT hour, AVG(people_count) as avg_count
            FROM traffic WHERE camera=?
            GROUP BY hour ORDER BY hour
        """, (camera,))
    else:
        c.execute("""
            SELECT hour, AVG(people_count) as avg_count
            FROM traffic GROUP BY hour ORDER BY hour
        """)
    rows = c.fetchall()
    conn.close()
    return rows
...
def get_daily_summary():
    ...
    c = conn.cursor()
    c.execute("""
        SELECT day, AVG(people_count), SUM(entries), SUM(exits)
        FROM traffic GROUP BY day
    """)
    rows = c.fetchall()
    conn.close()
    return rows
